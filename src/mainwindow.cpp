...
MainWindow::MainWindow(QWidget *parent)
    ...
    connect(ui->calendarWidget, &QCalendarWidget::selectionChanged, this, &MainWindow::onDateChanged);
    connect(ui->btnAddFood, &QPushButton::clicked, this, &MainWindow::onAddFoodClicked);
...
void MainWindow::loadDailyData() {
    ...
    QSqlQuery query;
    query.prepare("SELECT meal_type, food_name, weight, kcal, protein, fat, carbs FROM nutrition_log WHERE date_entry = :date ORDER BY meal_type");
    ...
    while (query.next()) {
        ...
        // Считаем общую сумму
        totalKcal += k; totalP += p; totalF += f; totalC += c;

        // Находим или создаем ветку (Завтрак/Обед...)
        QTreeWidgetItem *parentItem = getOrCreateMealCategory(type);

        // Добавляем продукт внутрь ветки
    ...
    // Обновляем надпись Итого
    ui->lblTotal->setText(QString("Итого за день: %1 ккал | Б: %2 | Ж: %3 | У: %4")
                              .arg(totalKcal).arg(totalP).arg(totalF).arg(totalC));
}
...
void MainWindow::onAddFoodClicked() {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        loadDailyData(); // Обновляем дерево после добавления
    }
}
