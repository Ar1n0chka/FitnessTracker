...
AddFoodDialog::AddFoodDialog(QDate date, QWidget *parent) :
    QDialog(parent), ui(new Ui::AddFoodDialog), targetDate(date)
{
    ui->setupUi(this);

    // Названия должны совпадать с именами в AddFoodDialog.ui
    ...
    connect(ui->resultsTable, &QTableWidget::cellClicked, this, &AddFoodDialog::onTableClicked);
...
void AddFoodDialog::onSearchTextChanged(const QString &text) {
    ...
    QSqlQuery query;
    query.prepare("SELECT name, calories, protein, fat, carbs FROM food_library WHERE name LIKE :search");
    ...
    while (query.next()) {
        ...
        for(int i = 0; i < 5; ++i)
            ui->resultsTable->setItem(row, i, new QTableWidgetItem(query.value(i).toString()));
    }
}
...
void AddFoodDialog::onAddClicked() {
    ...
    DatabaseManager::addEntry(targetDate, ui->mealCombo->currentIndex() + 1,
                              ui->searchEdit->text(), weight,
                              selKcal * ratio, selProt * ratio, selFat * ratio, selCarbs * ratio);
    accept();
}
